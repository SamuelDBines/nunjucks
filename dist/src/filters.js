import { repeat, isObject, TemplateError, isString, _entries, getAttrGetter, toArray, groupBy, } from './lib';
...
export function center(str = '', width = 80) {
    if (str.length >= width)
        return str;
...
export function default_(val, def, bool = false) {
    if (bool)
        return def || val;
    return val || def;
}
export const d = default_;
export function dictsort(val, caseSensitive, by = 'value') {
    if (!isObject(val)) {
...
export const safe = (str = '') => r.markSafe(str.toString());
...
export function indent(str = '', width = 4, indentfirst) {
    if (str === '') {
        return '';
    }
    // let res = '';
...
export function length(str) {
    if (Array.isArray(str) || isString(str))
        return str.length;
    if (isObject(str))
...
export function round(val, precision = 0, method = 'round') {
...
export function slice(arr, slices, fillWith = false) {
...
export const sort = r.makeMacro(['value', 'reverse', 'case_sensitive', 'attribute'], [], function sortFilter(arr, reversed = false, caseSens = false, attr) {
    // Copy it
...
export const string = (obj) => r.copySafeness(obj, obj);
export function striptags(input = '', preserveLinebreaks = true) {
...
export function truncate(input = '', length = 255, killwords = false, end) {
    let orig = input;
...
export const isUpper = (str) => str.toUpperCase() === str;
...
export const wordcount = (str = '') => str.match(/\w+/g)?.length || null;
...
export const int = r.makeMacro(['value', 'default', 'base'], [], function doInt(value, defaultValue = 0, base = 10) {
    var res = parseInt(value, base);
    return isNaN(res) ? defaultValue : res;
});
export const trim = (str) => r.copySafeness(str, str.replace(/^\s*|\s*$/g, ''));
// Aliases
export const first = (arr = []) => arr[0];
export const last = (arr = []) => arr[arr.length - 1];
exports.replace = replace;
export const lower = (str) => normalize(str).toLowerCase();
