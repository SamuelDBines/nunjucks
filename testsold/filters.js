(function () {
	...
	if (typeof require !== 'undefined') {
		...
		util = require('./util');
		lib = require('../dist/src/lib');
		r = require('../dist/src/runtime');
	} else {
	...
	describe('filter', function () {
		...
		it('center', function (done) {
			...
			equal(
				'{{ "foo" | center }}',
				lib.repeat(' ', 38) + 'foo' + lib.repeat(' ', 39)
			);
			finish(done);
		});
		...
		it('should work with non-string values', function () {
			...
			equal('{{ foo | escape }}', { foo: null }, { autoescape: false }, '');
		});
		...
		it('replace', function (done) {
			...
			equal(
				'{{ "<img src=" | replace("<img", "<img alt=val") | safe }}',
				'<img alt=val src='
			);
			equal(
		...
		it('sort', function (done) {
			...
			equal(
				'{% for i in ["fOo", "Foo"] | sort %}{{ i }}{% endfor %}',
				'fOoFoo'
			);
			...
			equal(
				'{% for i in ["fOo", "Foo"] | sort(false, true) %}{{ i }}{% endfor %}',
				'FoofOo'
			);
			...
			equal(
				'{% for i in [ {n:3},{n:5},{n:2},{n:1},{n:4},{n:6}] | sort(attribute="n") %}{{ i.n }}{% endfor %}',
				'123456'
			);

			const nestedAttributeSortTemplate =
		...
		it('urlize', function (done) {
			...
			equal(
				'{{ "pocoo.com:80" | urlize | safe }}',
				'<a href="http://pocoo.com:80">pocoo.com:80</a>'
			);
			equal(
				'{{ "pocoo.com" | urlize | safe }}',
