(function () {
	...
	if (typeof require !== 'undefined') {
		...
		nodes = require('../dist/src/nodes');
		parser = require('../dist/src/parser');
	} else {
		...
		if (node2 instanceof nodes.NodeList) {
			...
			node2.iterFields(function (value, field) {
				...
				if (value instanceof nodes.Node) {
					...
					expect('num-children: ' + ofield.length).to.be(
					...
					if (ofield === null) {
						throw new Error(
							value + ' expected for "' + field + '", null found'
						);
					}

					if (value === null) {
						throw new Error(
							ofield + ' expected to be null for "' + field + '"'
						);
					}
	...
	describe('parser', function () {
		...
		it('should parse imports', function () {
			...
			isAST(
				parser.parse(
					'{% from "foo/bar.njk" import baz, foobar as foobarbaz %}'
				),
				[
					nodes.Root,
					[
			...
			isAST(
				parser.parse(
					'{% import "foo/bar.html"|replace("html", "j2") as baz %}'
				),
				[
					nodes.Root,
					[
